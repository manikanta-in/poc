  ...
  return (
    <NotesTableWrapper>
      ...
      <TableStyleContaner component={Paper} style={{ border: 'none', boxShadow: 'none' }}>
        <div ref={tableRef} style={{ height: '365px', overflow: 'auto', marginTop: 11 }}>
              ...
              {notesData &&
                notesData?.Records?.map((row, index) => (
                  <StyledTableRow key={index}>
                    ...
                    <StyledTableCell align='left'>
                      {row.Description.length >= 60 ? (
                        <div style={{ maxWidth: '800px' }}>
                          ...
                          <OverFlowCharacters>
                            {showMore ? row.Description : `${row.Description.substring(0, 70)}...`}
                          </OverFlowCharacters>
                        </div>
                      ) : (
                        row.Description
                    ...
                    <StyledTableCell align='left'>{row.AddedBy}</StyledTableCell>
                  </StyledTableRow>
                ))}
